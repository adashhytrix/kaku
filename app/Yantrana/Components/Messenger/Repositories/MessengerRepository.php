...
class MessengerRepository extends BaseRepository implements MessengerRepositoryInterface
{
    ...
    public function fetchConversations($userId, $optionalLoggedInUserId = null)
    {
        ...
        // Join user_profiles for message_to_user
            ...
            ->select(
                \__nestedKeyValues([
                    ...
                    'message_to_user' => [
                      ...
                      // Select nicknames from user_profiles
                'message_from_user_profile' => [
                    'nickname AS message_from_user_nickname',
                ],
                'message_to_user_profile' => [
                    'nickname AS message_to_user_nickname',
                ],
                ])
            )
            ->get();
    ...
    public function fetchLastMessageBetweenUsers($senderId, $receiverId)
{
    // Fetch the last message where either the logged-in user is the sender or the receiver
    return DB::table('chats')->where(function ($query) use ($senderId, $receiverId) {
        $query->where('from_users__id', $senderId)
              ->where('to_users__id', $receiverId);
    })
    ->orWhere(function ($query) use ($senderId, $receiverId) {
        $query->where('from_users__id', $receiverId)
