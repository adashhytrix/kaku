...
//     DB::table('users')->where('_id', $userId)->update([
//         "first_name" => $inputData['first_name'],
...
//                 'specification_value' => json_encode($inputValue),
//                 'users__id' => $userId,
//                 '_id' => $userSpecifications[$inputKey] ?? null, // If exists, update; otherwise, insert
//             ];
//         }
//     }

//     // Store or update user specifications
...
public function processStoreUserProfileSetting($inputData)
{
    ...
    // Initialize the update array for the users table
    $userUpdateData = [];

    // Only add fields to the update array if they exist in the input data
    if (isset($inputData['first_name'])) {
        $userUpdateData['first_name'] = $inputData['first_name'];
    }
    if (isset($inputData['username'])) {
        $userUpdateData['username'] = $inputData['username'];
    }
    if (isset($inputData['email'])) {
        $userUpdateData['email'] = $inputData['email'];
    }
    if (isset($inputData['mobile_number'])) {
        $userUpdateData['mobile_number'] = $inputData['mobile_number'];
    }

    // Update basic user information if any fields are present
    if (!empty($userUpdateData)) {
        DB::table('users')->where('_id', $userId)->update($userUpdateData);
    }
    ...
    if ($this->userSettingRepository->storeOrUpdateUserSpecification($storeOrUpdateData)) {
        $userInfo = getUserAuthInfo();
        $fullName = array_get($userInfo, 'profile.full_name');
        activityLog($fullName.' updated their user settings.');



        // social media update 
        // if (isset($inputData['Whatsapp'])) {
        //     DB::table('contact_support')->where('id',1)->update([
    
        //         'contact_value'=>$inputData['Whatsapp']
        //     ]);
        // }
        if (isset($inputData['instagram'])) {
            DB::table('contact_support')->where('id',2)->update([
    
                'contact_value'=>$inputData['instagram']
            ]);
        }
        if (isset($inputData['website'])) {
            DB::table('contact_support')->where('id',3)->update([
    
                'contact_value'=>$inputData['website']
            ]);
        }
        if (isset($inputData['facebook'])) {
            DB::table('contact_support')->where('id',4)->update([
    
                'contact_value'=>$inputData['facebook']
            ]);
        }
        if (isset($inputData['Twitter'])) {
            DB::table('contact_support')->where('id',5)->update([
    
                'contact_value'=>$inputData['Twitter']
            ]);
        }
        return $this->engineReaction(1, null, __tr('Profile updated successfully.'));
    }

    return $this->engineReaction(14, null, __tr('Profile updated successfully.'));
}
    ...
    //     DB::table('users')->where('_id', $userId)->update([
    //         "first_name" => $inputData['first_name'], // Corrected this line
    //         "username" => $inputData['username'], 
    //         "email" => $inputData['email'], 
    //         "mobile_number" => $inputData['mobile_number'], 
