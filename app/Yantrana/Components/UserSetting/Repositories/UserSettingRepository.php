...
class UserSettingRepository extends BaseRepository implements UserSettingRepositoryInterface
{
    ...
    // }
    public function fetchFilterData($filterData, $paginateCount = false)
    {
        if (!$paginateCount) {
            $paginateCount = configItem('user_settings.search_pagination');
        }
        ...
        $searchQuery = UserProfile::join('users', 'user_profiles.users__id', '=', 'users._id')
            ...
            ->select(
                ...
                'user_specifications.specification_key',
                'user_specifications.specification_value'
            );
    
        $searchInput = $filterData['username'] ?? ''; // Username search input
        $interestArray  = $filterData['intrest'] ?? []; // Interest search input
    
        // Filter by username/nickname
        $searchQuery->where(function ($query) use ($searchInput) {
            $query->where('user_profiles.nickname', 'LIKE', '%' . $searchInput . '%')
                  ->orWhere('users.username', 'LIKE', '%' . $searchInput . '%');
        });
    
        // Filter by interest if provided
        // if (!empty($interest)) {
        //     $searchQuery->where(function ($query) use ($interest) {
        //         $query->where('user_specifications.specification_key', 'intrest')
        //               ->where('user_specifications.specification_value', 'LIKE', '%' . $interest . '%');
        //     });
        // }
        if (is_string($interestArray)) {
            $interestArray = json_decode($interestArray, true);
        }
        
        
        // Ensure $interestArray is an array before using in the foreach
        if (is_array($interestArray)) {
            // Now you can safely use foreach on $interestArray
            foreach ($interestArray as $interest) {
                // Perform your logic with each interest
                $searchQuery->orWhere('user_specifications.specification_value', 'LIKE', '%' . $interest . '%')
                            ->where('user_specifications.specification_key', 'intrest');
            }
        }
    
        // Group by user to avoid duplication due to joins
        $searchQuery->groupBy('users._id');
    
        // Execute query and paginate results
        return $searchQuery->latest('user_profiles.updated_at')->paginate($paginateCount);
    }
