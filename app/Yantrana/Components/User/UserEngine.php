...
class UserEngine extends BaseEngine
{
     ...
     *
     *-----------------------------------------------------------------------*/
    public function __construct(BaseMailer $baseMailer, UserRepository $userRepository, UserSettingRepository $userSettingRepository, ManageItemRepository $manageItemRepository, CreditWalletRepository $creditWalletRepository, ManageAbuseReportRepository $manageAbuseReportRepository, UserEncounterRepository $userEncounterRepository, CountryRepository $countryRepository, MediaEngine $mediaEngine, LoginLogsRepository $loginLogsRepository, CreditWalletEngine $creditWalletEngine)
    {
    ...
    //         return $this->engineReaction(2, ['show_message' => true], __tr('Invalid Re-captcha.'));
    //     }
    //     $existingUser = $this->userRepository->fetchByEmailOrUsername($inputData['email']);
    //     if ($existingUser) {
    //         return $this->engineReaction(2, ['show_message' => true], __tr('Email already exists.'));
    //     }
    ...
    //                         return $this->userRepository->transactionResponse(
    //                             1,
    //                             [
    //                                 'show_message' => true,
...
//             'otp' => $otp,
//         ];
//         $this->baseMailer->notifyToUser('Your OTP for login', 'account.otp', $emailData, $newUser->email);

//         return $this->userRepository->transactionResponse(1, ['otp_required' => true], __tr('OTP sent to your email. Please enter it to verify.'));
//     });
...
public function userSignUpProcess($inputData)
{
    ...
    $transactionResponse = $this->userRepository->processTransaction(function () use ($inputData) {
        ...
        $activationRequiredForNewUser = getStoreSettings('activation_required_for_new_user');
        $inputData['status'] = 1; // Active by default
        ...
        // Now store user profile in the 'user_profile' table
        $profileData = [
            'users__id' => $newUser->_id,
            // Add other fields from $inputData, e.g., 'gender', 'dob', etc.
            'status' => 1,
        ];

        // Store profile data
        if (!$this->userRepository->storeUserProfile($profileData)) {
            return $this->userRepository->transactionResponse(2, ['show_message' => true], __tr('User profile not added.'));
        }
...
public function signup_otp($inputData)
{
    // Validate input data if needed
    // Uncomment the validation if it's required
    // $inputData->validate([
    ...
    $otpRecord = DB::table('otps')
        ...
        ->where('otp', $inputData['otp'])
        ->where('expires_at', '>', Carbon::now())
        ->first();

    if (!$otpRecord) {
        return $this->engineReaction(2, ['show_message' => true], __tr('Invalid or expired OTP.'));
    }
    ...
    // Optionally send a welcome email if the setting is enabled
    if (getStoreSettings('send_welcome_email_to_newly_registered_users')) {
        $this->userWelcomeNotifyMail($user);
    }
    ...
    $responseData = [
        ...
        'access_token' => $authToken,
        'intendedUrl' => Session::get('intendedUrl'), // Ensure `intendedUrl` is set
        'show_message' => true,
    ];
    ...
    public function processUpdatePassword($inputData)
    {
        ...
        $hashed=Hash::make($inputData['current_password']);
        // dd($hashed.'---------'.$user->password);
    ...
    public function prepareUserProfile($userName)
    {
        ...
        if (!\__isEmpty($userProfile)) {
            ...
            $peopleILikeUserIds = $this->userRepository
                ->fetchMyLikeDataByUserId($user->_id)
                ->pluck('to_users__id')
                ->toArray();
            ...
            $userProfileData = [
                ...
                'dob' => $userProfile->dob,
                'birthday' => !\__isEmpty($userProfile->dob) ? formatDate($userProfile->dob) : '',
            ...
            foreach ($specification['items'] as $itemKey => $item) {
                if (!$isOwnProfile and array_key_exists($itemKey, $userSpecifications)) {
                    ...
                    $items[] = [
                        ...
                        'value' => (isset($item['options']) and isset($item['options'][$userSpecKey])) ? $item['options'][$userSpecKey] : $userSpecifications[$itemKey],
                        'options' => isset($item['options']) ? $item['options'] : '',
                        'selected_options' => !__isEmpty($userSpecKey) ? $userSpecKey : '',
                    ];
                } elseif ($isOwnProfile) {
                    $itemValue = '';
                    $userSpecValue = isset($userSpecifications[$itemKey]) ? $userSpecifications[$itemKey] : '';
                    if (!__isEmpty($userSpecValue)) {
                        $itemValue = isset($item['options']) ? (isset($item['options'][$userSpecValue]) ? $item['options'][$userSpecValue] : '') : $userSpecValue;
                    }
                    $items[] = [
                        'name' => $itemKey,
                        'label' => $item['name'],
    ...
    public function processUserSendGift($inputData, $sendUserUId)
    {
        ...
        $userSendGiftRequest = $this->userRepository->processTransaction(function () use ($inputData, $sendUserUId) {
            ...
            if ($creditWalledId = $this->userRepository->storeCreditWalletTransaction($creditWalletStoreData)) {
                //store gift data
                $giftStoreData = [
                    'status' => (isset($inputData['isPrivateGift']) and $inputData['isPrivateGift'] == 'on') ? 1 : 0,
        ...
        $transactionResponse = $this->userRepository->processTransaction(function () {
            ...
            if ($creditWalledId = $this->userRepository->storeCreditWalletTransaction($creditWalletStoreData)) {
                ...
                if ($booster = $this->userRepository->storeBooster($boosterData)) {
                    //activity log message
                    activityLog(
                        strtr('Booster activated by user __firstName__ __lastName__', [
    ...
    public function checkProfileStatus()
    {
        ...
        if ($userProfile->status == 2) {
            ...
            if ((__isEmpty($userProfile['location_longitude']) or $userProfile['location_longitude'] == 0) and (__isEmpty($userProfile['location_latitude']) or $userProfile['location_latitude'] == 0)) {
    ...
    public function requestNewPassword($inputData)
    {
        ...
        // Check if user exists
        if (__isEmpty($user)) {
            \Log::error('User not found for email: ' . $inputData['email']);
            return $this->engineReaction(18, ['show_message' => true], __tr('You are not a member of this system.'));
        }

        // Check if user is active
        if ($user->status != 1) {
            \Log::error('Inactive, blocked, or not activated user: ' . $inputData['email']);
            return $this->engineReaction(18, ['show_message' => true], __tr('Your account might be Inactive, Blocked, or Not Activated.'));
        }
        ...
        $transactionResponse = $this->userRepository->processTransaction(function () use ($inputData, $user) {
            ...
            \Log::info('Old password reminder deleted for email: ' . $inputData['email']);
            ...
            // Store password reminder
            if (!$this->userRepository->storePasswordReminder($storeData)) {
                \Log::error('Failed to store password reminder for email: ' . $inputData['email']);
                return $this->userRepository->transactionResponse(2, ['show_message' => true], __tr('Invalid Request.'));
            }
            \Log::info('Password reminder stored for email: ' . $inputData['email']);
            ...
            try {
                // Send email
                Mail::send('emails.account.forgot-password-for-app', $emailData, function ($message) use ($user) {
                ...
                $newExpiryTime = Carbon::now()->diffInSeconds($createdAt, false);
    ...
    public function prepareProfileUpdate()
    {
        ...
        // Check if user exists
        if (__isEmpty($user)) {
            return $this->engineReaction(18, ['show_message' => true], __tr('User does not exist.'));
        }
        ...
        $basicInformation = [
            ...
            'profileMediaRestriction' => getMediaRestriction('profile'),
            'coverImageMediaRestriction' => getMediaRestriction('cover_image'),
            'nickname' => $userprofile->nickname != null ? $userprofile->nickname : 'nickname',
            'relationshipgoal' => $userprofile->relationshipgoal,
            'nearby' => $userprofile->nearby,
            'faith' => json_decode($userprofile->faith),
            'wonderlust' => json_decode($userprofile->wonderlust),
            'discover' => json_decode($userprofile->discover),
            'about_me' => $userprofile->about_me,
            'dob' => $userprofile->dob,
            'about_me' => $userprofile->about_me,
            'profile_complete'=>$userprofile->is_profile_complete==0 ? false:true,
        ];
        ...
        $totalFields = 16; // Total number of fields considered for profile completion
        $completedFields = 0;
    
        // Increment completed fields based on non-empty values
        ...
        foreach ($locationInformation as $key => $value) {
            if (!is_null($value) && $value !== '') {
                $completedFields++;
            }
        }
    
        foreach ($userSpecifications as $key => $value) {
            if (!is_null($value) && $value !== '') {
                $completedFields++;
            }
        }
        ...
        return $this->engineReaction(1, [
            ...
            // 'genders' => $genders,
            'profileCompletionPercentage' => round($profileCompletionPercentage, 2), // Round to 2 decimal places
        ]);
    }
    ...
    //     // Check if the input OTP is '1234'
    //     if ($inputData['otp'] === '1234') {
    //         // OTP is valid, handle the successful verification
    //         $emailOrMobile = $inputData['emailOrMobile']; // Retrieve email or mobile number from input data
    //         $user = $this->userRepository->fetchByEmailOrUsername($emailOrMobile);
    ...
    //         }
    //     } else {
    //         // If the OTP is not '123', return an error response
    //         return $this->engineReaction(2, ['show_message' => true], __tr('Invalid OTP.'));
    //     }
    // }
    ...
    // public function processProfileUpdate($inputData)
    // {
    //     $userProfile = $this->userSettingRepository->fetchUserProfile(getUserID());

    //     if ($userProfile) {
    //         $userProfile->dob = $inputData['dob'];
    //         $userProfile->nickname = $inputData['nickname'];
    //         $userProfile->relationshipgoal = $inputData['relationshipgoal'];
    //         $userProfile->nearby = $inputData['nearby'];
    //         $userProfile->gender = $inputData['gender'];
    //         // $userProfile->religion =$inputData['religion']; used in user specification table
    //         $userProfile->faith = $inputData['faith'];
    //         $userProfile->wonderlust = $inputData['wonderlust'];
    //         $userProfile->discover = $inputData['discover'];

    //         $userProfile->save();

    //         return $this->engineReaction(6, null, __tr('user profile Updated Successfully'));
    //     } else {
    //         // Return an error response if the user profile is not found
    //         return $this->engineReaction(4, null, __tr('failed'));
    //     }
    // }
    public function processProfileUpdate($inputData)
    {
    ...
    // dd(getUserID());
        if ($userProfile) {
            ...
            // Fetch existing user specifications
            $userSpecificationCollection = $this->userSettingRepository->fetchUserSpecificationById($userId);
    
            if (!$userSpecificationCollection->isEmpty()) {
                $userSpecifications = $userSpecificationCollection->pluck('_id', 'specification_key')->toArray();
            }
    
            // Handle 'intrest' field
            if (!empty($inputData['intrest'])) {
                // Prepare the data to insert or update
                $storeOrUpdateData[] = [
                    'type' => 1,
                    'status' => 1,
                    'specification_key' => 'intrest', // Correct key for interest
                    'specification_value' => json_encode($inputData['intrest']), // Storing interest as JSON
                    'users__id' => $userId,
                    '_id' => $userSpecifications['intrest'] ?? null, // If exists, update; otherwise, insert
                ];
            }
    
            // Store or update the user specifications
            if (!empty($storeOrUpdateData)) {
                $this->userSettingRepository->storeOrUpdateUserSpecification($storeOrUpdateData);
            }
    
            return $this->engineReaction(6, null, __tr('User profile updated successfully.'));
        } else {
            // Return error if user profile is not found
            return $this->engineReaction(4, null, __tr('Failed to update user profile.'));
        }
    }
    ...
    {
        $relationship = DB::table('relationshipgoal')->orderBy('id', 'Desc')->get();
        return $this->engineReaction(1, ['relation' => $relationship]);
    }
}
